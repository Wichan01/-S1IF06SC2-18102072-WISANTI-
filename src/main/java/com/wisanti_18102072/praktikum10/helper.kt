    ...
    fun mapCursorToArrayList(notesCursor: Cursor?): ArrayList<Quote> {
        ...
        notesCursor?.apply {
            while (moveToNext()) {
                ...
                //val medsos = getString(getColumnIndexOrThrow(DatabaseContract.QuoteColumns.MEDSOS))
                //val number = getString(getColumnIndexOrThrow(DatabaseContract.QuoteColumns.NUMBER))
